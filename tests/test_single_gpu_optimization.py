...
def _attention_and_lse(q, k, v, is_causal=False):
    ...
    if is_causal:
        S = torch.where(
            torch.arange(n_queries, device=S.device)[None, :, None] >= torch.arange(n_keys, device=S.device)[None, None, :], S, -1e6)
...
def _make_attn_inputs(device=None):
    ...
    batch_size, n_queries, n_keys, D = 4, 128, 128, 64
...
def _test_flash_forward_pass(impl, device='cpu', is_causal=False):
    ...
    assert len(maybe_ls) == 1, (
        ...
        'The tests require you to save exactly one tensor of this shape, corresponding to the log-sum-exp of the attention scores.'
    )
    l = maybe_ls[0]
...
def _flash_backward_results(impl, is_causal, device=None):
    q, k, v, do = _make_attn_inputs(device=device)
    impl(q, k, v, is_causal).backward(do)
    return q.grad, k.grad, v.grad

def test_flash_forward_pass_pytorch(get_flashattention_autograd_function_pytorch):
    _test_flash_forward_pass(get_flashattention_autograd_function_pytorch().apply)
    print('PASSED: test_flash_forward_pass_pytorch')

def test_flash_forward_pass_triton(get_flashattention_autograd_function_triton):
    if not torch.cuda.is_available():
        print('SKIPPED: test_flash_forward_pass_triton (no GPU available)')
        return
    for is_causal in [False, True]:
        _test_flash_forward_pass(get_flashattention_autograd_function_triton().apply, device='cuda', is_causal=is_causal)
    print('PASSED: test_flash_forward_pass_triton')

def test_flash_backward_pytorch(get_flashattention_autograd_function_pytorch):
    dq_expected, dk_expected, dv_expected = _flash_backward_results(lambda *args: _attention_and_lse(*args)[0], False)
    ...
    print('PASSED: test_flash_backward_pytorch')

def test_flash_backward_triton(get_flashattention_autograd_function_triton):
    if not torch.cuda.is_available():
        print('SKIPPED: test_flash_backward_triton (no GPU available)')
        return
    for is_causal in [False, True]:
        dq_expected, dk_expected, dv_expected = _flash_backward_results(lambda *args: _attention_and_lse(*args)[0], is_causal, device='cuda')
        q, k, v, do = _make_attn_inputs(device='cuda')
        get_flashattention_autograd_function_triton().apply(q, k, v, is_causal).backward(do)
        torch.testing.assert_close(dq_expected, q.grad, rtol=1e-2, atol=1e-2)
        torch.testing.assert_close(dk_expected, k.grad, rtol=1e-2, atol=1e-2)
        torch.testing.assert_close(dv_expected, v.grad, rtol=1e-2, atol=1e-2)
    print('PASSED: test_flash_backward_triton')
